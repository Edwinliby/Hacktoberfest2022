    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) 
    {
      ...
      if(n > m )
        ...
        while(low <= high)
        {
            ...
            int maxleft_x  = divx == 0 ? INT_MIN : nums1[divx - 1];
            int minright_x = divx == n ? INT_MAX : nums1[divx ];
            
            int maxleft_y  = divy == 0 ? INT_MIN : nums2[divy - 1];
            int minright_y = divy == m ? INT_MAX : nums2[divy ];
